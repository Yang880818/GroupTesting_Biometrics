...
set.seed(seed.num)
N = 12420
Data_resample <- read.csv("./Data/Data_WholeBlood_final_test.csv", header = T)
dim(Data_resample)
sum(Data_resample$Disease_status[which(Data_resample$Disease_status == 1)])
Data_resample[0, ]
colnames(Data_resample) <- c("X", "SEQN","Disease_status", "Biomarker_1", "Biomarker_2", "Biomarker_3", "Weight")
Data_resample_final <- Data_resample[sample(nrow(Data_resample), N, replace = FALSE), ]


Biomarker_1 <- Data_resample_final$Biomarker_1
Biomarker_2 <- Data_resample_final$Biomarker_2
Biomarker_3 <- Data_resample_final$Biomarker_3

Biomarker_1 <- scale(Biomarker_1, center = FALSE, scale = TRUE)
Biomarker_2 <- scale(Biomarker_2, center = FALSE, scale = TRUE)
Biomarker_3 <- scale(Biomarker_3, center = FALSE, scale = TRUE)
# Biomarker_4_0 <- scale(Biomarker_4_0, center = FALSE, scale = TRUE)
# Biomarker_4_1 <- scale(Biomarker_4_1, center = FALSE, scale = TRUE)
...
plot(aa, ROC.true_3, type = "l", xlab = "u", ylab = "ROC", mgp = c(1.5,0.5,0), cex.lab = 2, lwd = 2, cex.axis = 1.5)
abline(0, 1, col = "red", lwd = 1, lty = 1)
...
write.csv(final_results, file = paste0("results/LogTrans_3Bio_WholeBlood_K=", K, "_", pi0, "_", pi1, "_", CurrentData, ".csv"))
...
Main_Fun <- foreach(j = 1 : seed.num.boot, .packages= c('MASS', 'Rfast', 'bbmle', 'combinat')) %dopar%
  {
    ...
    for(l in 1 : rho_leg)
    {
      ...
      # EstPMeanSD.boot[j, ] <- as.vector(optim(c(mu0_comb[1,l], mu0_comb[2,l], mu1_comb[1,l], mu1_comb[2,l], sd0_comb[1,l], sd0_comb[2,l], sd1_comb[1,l], sd1_comb[2,l], rho0[l], rho1[l], p),
      #                                         LoglikPMeanSD_est.boot, pi0 = pi0, pi1 = pi1, N = N, n = n, K = K, XYvec = XYvec.boot, lambda = lambda,
      #                                         groupM = groupM.boot, aa.mat = aa.mat, pi1_lik.matrix = pi1_lik.matrix)$par)
    ...
    write.table(AUC.value.boot[j], file = paste0("results/Boot/AUC_boot/LogTrans_resample_3Bio_WholeBlood_Seed.boot_", j , "_", "K=", K, "_", CurrentData, ".txt"),
                sep="\t", row.names=F)
    ...
    # Est_Bio1.boot[j, ] <- as.vector(optim(c(mu01, mu11, sd01, sd11, p), Loglik_Bio1.boot, pi0 = pi0, pi1 = pi1, N = N, n = n, K = K, XYvec = XYvec_Bio1,
    #                                       lambda = lambda, groupM = groupM.boot, aa.mat = aa.mat, pi1_lik.matrix = pi1_lik.matrix)$par)
    ...
    Est_Bio2.boot[j, ] <- as.vector(optim(c(mu02.boot, mu12.boot, sd02.boot, sd12.boot, p.boot), Loglik_Bio2.boot, pi0 = pi0, pi1 = pi1, N = N, n = n, K = K, XYvec = XYvec_Bio2,
                                          lambda = lambda, groupM = groupM.boot, aa.mat = aa.mat, pi1_lik.matrix = pi1_lik.matrix)$par)
    # Est_Bio2.boot[j, ] <- as.vector(optim(c(mu02, mu12, sd02, sd12, p), Loglik_Bio2.boot, pi0 = pi0, pi1 = pi1, N = N, n = n, K = K, XYvec = XYvec_Bio2,
    #                                       lambda = lambda, groupM = groupM.boot, aa.mat = aa.mat, pi1_lik.matrix = pi1_lik.matrix)$par)
    ...
    Est_Bio3.boot[j, ] <- as.vector(optim(c(mu03.boot, mu13.boot, sd03.boot, sd13.boot, p.boot), Loglik_Bio3.boot, pi0 = pi0, pi1 = pi1, N = N, n = n, K = K, XYvec = XYvec_Bio3,
                                          lambda = lambda, groupM = groupM.boot, aa.mat = aa.mat, pi1_lik.matrix = pi1_lik.matrix)$par)
    # Est_Bio3.boot[j, ] <- as.vector(optim(c(mu03, mu13, sd03, sd13, p), Loglik_Bio3.boot, pi0 = pi0, pi1 = pi1, N = N, n = n, K = K, XYvec = XYvec_Bio3,
    #                                       lambda = lambda, groupM = groupM.boot, aa.mat = aa.mat, pi1_lik.matrix = pi1_lik.matrix)$par)
...
write.csv(sim_stats, file = paste0("results/Boot/LogTrans_3Bio_WholeBlood_Seed.boot=", seed.num.boot, "_", "K=", K, "_", Pi0, "_", Pi1, "_", CurrentData, ".csv"))
